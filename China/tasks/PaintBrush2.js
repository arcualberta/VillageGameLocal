(function(){
	...
	comparisonImage.complete = false;
	comparisonImage.onload = function(){
		comparisonImage.complete = true;
	};
	
	// Draw the brush to a separate canvas.
	var drawCanvas = document.createElement("canvas");
	var drawContext = drawCanvas.getContext("2d");
	
	var drawBrush = function(display, model, drawModel){
		var size = display.size;
		if(display.size[0] != drawCanvas.width || display.size[1] != drawCanvas.height){
		...
		if(line && line.time > 0){
			...
			for(var i = 0; i < dotCount; ++i){
				...
				drawContext.drawImage(display.brushImage, 0, 0, display.brushImage.width, display.brushImage.height, 
	...
	var result = {
		draw: function(display, model, drawModel){
			...
			if(this.bgImage){
				display.drawImage(this.bgImage, 0, 0, 1, 1,
				0, 0, display.size[0], drawModel.bottom);
				
				display.drawImage(this.bgImage, 0, 0, this.bgImage.width, this.bgImage.height,
				0, 0, display.size[0], drawModel.bottom);
			}

			if(display.brushImage && display.brushImage.complete){
				drawBrush(display, model, drawModel);
